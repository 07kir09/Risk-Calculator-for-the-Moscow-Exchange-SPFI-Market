...
### UI (редизайн 4.1): где что находится
- Shell и навигация: `cod/frontend/src/layout/AppShell.tsx`
- Дизайн-система (базовые компоненты): `cod/frontend/src/ui/`
  - `Card.tsx`, `PageHeader.tsx`, `Section.tsx`, `FormField.tsx`, `ResultPanel.tsx`
  - `DataTable.tsx`, `StatePanel.tsx`, `KpiCard.tsx`, `SegmentedControl.tsx`
  - `AdvancedSettings.tsx`, `Skeleton.tsx`, `ErrorBoundary.tsx`, `Toast.tsx`
- Ключевые экраны:
  - Home / Overview: `cod/frontend/src/pages/OverviewPage.tsx`
  - Импорт/управление входом: `cod/frontend/src/pages/ImportPage.tsx`
  - Валидация: `cod/frontend/src/pages/ValidatePage.tsx`
  - Настройка расчёта: `cod/frontend/src/pages/ConfigurePage.tsx`
  - Запуск расчёта (single/portfolio): `cod/frontend/src/pages/RunPage.tsx`
  - Результаты (KPI + графики + таблицы): `cod/frontend/src/pages/DashboardPage.tsx`
  - Сценарии и сравнение запусков: `cod/frontend/src/pages/ScenariosPage.tsx`
  - Экспорт отчётов: `cod/frontend/src/pages/ExportPage.tsx`
- Глобальные стили и responsive-поведение: `cod/frontend/src/styles.css`

Маршруты product-уровня:
- `/overview`, `/calculator`, `/portfolio`, `/scenarios`, `/results`, `/reports`
- workflow-роуты сохранены: `/import`, `/validate`, `/market`, `/configure`, `/run`, `/dashboard`, `/stress`, `/limits`, `/margin`, `/export`

### Как расширять UI
- Добавляйте новые визуальные блоки через `ui/` примитивы, а не инлайн-стили в страницах.
- Для пустых/ошибочных/загрузочных состояний используйте `StatePanel`.
- Для форм c единицами и ошибками используйте `FormField`.
- Для новых KPI на дашборде используйте `KpiCard`.
- Для табличных экранов используйте `DataTable` и управляйте плотностью через `compact`.
- Для расширенных параметров используйте `AdvancedSettings` (collapsed by default).
- Для переключателей режима используйте `SegmentedControl`.
- Для пользовательского фидбека (экспорт/ошибки/успех) используйте `useToast()`.
...
- переключатель светлой/тёмной темы в шапке workspace;
