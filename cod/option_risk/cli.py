...
def _load_portfolio(path: Path) -> tuple[Portfolio, List[ValidationMessage]]:
...
def main() -> None:
    ...
    portfolio, validation_log = _load_portfolio(portfolio_path)
    ...
    if validation_log:
        ...
        for msg in validation_log:
            print(f"- [{msg.severity}] строка {msg.row}: {msg.message}")
